...
namespace lks.uc
{
    public partial class ucTransaksi : UserControl
    {
        ...
        {
            if (transaksi.keranjangs.Count > 0)
            {
                BindingSource bs = new BindingSource();
                bs.DataSource = transaksi.keranjangs;
                dgKeranjang.DataSource = bs;
                lblTotal.Text = transaksi.keranjangs.Sum(x => x.total_harga).ToString();
            }
            else
            {
        ...
        {
            int total = transaksi.keranjangs.Sum(x => x.total_harga);
