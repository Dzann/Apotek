...
namespace lks.uc
{
    public partial class ucLaporan : UserControl
    {
        ...
        {
            string sql = $@"SELECT tgl_transaksi as ['Tgl Transaksi'], SUM(total_bayar) as ['Jumlah Pendapatan'] FROM transaksi WHERE tgl_transaksi BETWEEN '{dpFrom.Value.ToString("yyyy-MM-dd")}' AND '{dpTo.Value.ToString("yyyy-MM-dd")} 23:59:59' GROUP BY tgl_transaksi";
