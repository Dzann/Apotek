...
namespace lks.models
{
    class Transaksi
    {
        public List<Keranjang> keranjangs = new List<Keranjang>();
        ...
        public int saveData(List<Keranjang> _list)
        {
            ...
            string sql_transaksi = $"INSERT INTO transaksi values(@no_transaksi, @tgl_transaksi, @nama_kasir, @total_bayar, @obat_id, @user_id";
            int n = 0;
            string sq12 = "SELECT TOP 1 id FROM resep ORDER BY id DESC";
            string no_transaksi = "INV" + DateTime.Now.ToString("yyyy-MM-dd");
            foreach (var k in _list)
            {
                if (k.tipe_resep == "DENGAN RESEP")
                {
                    ...
                    var datas = Connect.getData(sq12);
